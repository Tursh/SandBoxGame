...
namespace SBG
{
    ...
    {
        shader.start();
        shader.loadMatrix(Shader::VIEW, camera_.toViewMatrix());
        ...
        for (auto &yChunks : chunks_)
            for (auto &zChunks : yChunks.second)
                for (auto &chunkPair : zChunks.second)
                {
                    ...
                    {
                        shader.loadMatrix(Shader::TRANSFORMATION,
        ...
        GUI::Text::TextRenderer::renderText(glm::to_string(camera_.position_), {-1, 0.95f}, 0.1f, glm::vec3(1, 1, 1),
                                            false);

        GUI::Text::TextRenderer::renderText("+", {0, 0}, 0.2f, glm::vec3(1, 1, 1),
                                            false);

        GUI::Text::TextRenderer::renderText(glm::to_string(player_->getSpeed()), {-1, 0.90f}, 0.1f,
                                            glm::vec3(1, 1, 1),
                                            false);
...
*/
        ...
        auto display = IO::getWindow();
        shader.start();

        shader.loadMatrix(Shader::PROJECTION,
            ...
            for (int y = negLimit.y; y < posLimit.y; ++y)
                for (int z = negLimit.z; z < posLimit.z; ++z)
                {
                    ...
                    if (getBlock({x, y, z}).ID != Blocks::AIR)
                        hitboxes.emplace_back((float) x, (float) x + 1, (float) y, (float) y + 1, (float) z,
                                              (float) z + 1);
                }
